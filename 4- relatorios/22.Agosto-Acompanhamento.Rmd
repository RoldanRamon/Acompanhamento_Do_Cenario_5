---
title: "22.Agosto - Acompanhamento do Cenário 5"
output:
  html_document:
    css: "style.css"
---
...
graf <- status_ocs_cenario_5 %>% group_by(data_inclusao,ordem_compra) %>% summarise(total=1) %>% ungroup() %>% group_by(data_inclusao) %>% summarise(total=sum(total)) %>% arrange(desc(data_inclusao))
  ...
  ylab('')+xlab('')+
    labs(caption = paste0('Produção média (azul): ',round(mean(graf$total),digits = 0)))+
  geom_text(aes(label=total),hjust=.1, angle=90,color='blue')

```

### OCs pendentes de tratativa por comprador
...
### OCs pendentes de liberação

```{r,echo=FALSE,warning=FALSE,message=FALSE}
DT::datatable(status_ocs_cenario_5 %>% filter(status %in% c('CADASTRADA')),
              caption = 'Estes processos aguardam liberação e confirmação da SOS.',
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
DT::datatable(status_ocs_cenario_5 %>% filter(status %in% c('CANCELADA','RECUSADA')),
              caption = 'Estes processos devem ser substituidos por outros, com status de cadastrada e/ou confirmada, porque ocs canceladas e/ou recusadas não permitem lançamento de NF',
              extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
    buttons = 
      list('copy', 'print', list(
